...
<div class="box">
  ...
  if (meth === 'POST' && p === '/api/switch/live') {
    ...
    // DeckLink-Verfügbarkeit hart erzwingen — Defekt (Platzhalter aktiv) UND "kein
    // Signal" blockieren den manuellen Cut gleichermaßen. Muss serverseitig geprüft
    // werden, nicht nur im Switcher-UI (Client-Sperre in ui.html switchLive() ist
    // nur UX-Komfort — StreamDeck & andere Aufrufer gehen direkt gegen diese Route).
    if (/decklink/i.test(ls.gstSrc || '')) {
      if (master.isLiveSourceBroken(ls.id)) {
        return json(res, { ok: false, error: 'DeckLink: Defekt — Platzhalter aktiv, Cut blockiert' });
      }
      if (master.getLiveSignalStatus(ls.id) === false) {
        return json(res, { ok: false, error: 'DeckLink: Kein Signal, Cut blockiert' });
      }
    }
