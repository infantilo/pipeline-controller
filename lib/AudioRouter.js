...
class AudioRouter {
   ...
   */
  static compactTracksForPreset(tracks, preset) {
    const fallback = { tracks, preset };
    if (!preset || !Array.isArray(preset.routes) || !preset.routes.length) return fallback;
    ...
    // If some preset routes reference channels beyond the file's track count, don't partially compact —
    // fall back to the full track set so _matrixFromRoutes can zero those outputs gracefully.
    if (usedIdx.length < used.size) return fallback;

    // Ummapping: globaler Index → kompakte Position (1-basiert)
    const remap = new Map(usedIdx.map((n, i) => [n, i + 1]));
    const compactRoutes = preset.routes.map(r => {
      const m = /^mxf_ch(\d+)$/.exec(r.from || '');
      const n = m ? remap.get(parseInt(m[1], 10)) : null;
      return n ? { ...r, from: `mxf_ch${n}` } : null;
    }).filter(Boolean);
    if (!compactRoutes.length) return fallback;
    ...
    return {
      tracks: [...byEssence.values()],
      preset: { ...preset, routes: compactRoutes },
    };
  }
