...
class AudioRouter {
   ...
   *
   * DeckLink/PTP (ClockStrategy.provider = 'ptp-decklink'):
   *   sinkCfg.type='decklink' → decklinkaudiosink sync=true als Clock-Provider
   *   (gst-kit hat keine setClock-API — Clock kommt via Sink-Auto-Selection).
   *   sinkCfg.noProvideClock → provide-clock=false auch im alsasink-Pfad erzwingen
   *   (der DeckLink-Sink muss der EINZIGE Clock-Provider bleiben).
   *
   * @param {object}      group       — {id, channels}
   * @param {object|null} sinkCfg     — {type, bufferTime, device, audioSink, noProvideClock}
  ...
  static _buildSink(group, sinkCfg, isPrimary = false) {
    ...
    const pc   = (isPrimary && !sinkCfg?.noProvideClock) ? 'true' : 'false';
    ...
    // ClockStrategy 'ptp-decklink' mit explizitem decklinkDevice: primäre Gruppe geht
    // auf decklinkaudiosink. sync=true — der Sink ist Clock-Provider der Master-Pipeline
    // (Sink-Clock-Auto-Selection, Karten-Clock ist auf IP100 PTP-locked), die Hardware
    // taktet die Ausgabe. Caps laut Sink-Template: S16LE/S32LE, rate=48000, ch {2,8,16}.
    if (sinkCfg.type === 'decklink') {
      const dn = parseInt(sinkCfg.device) || 0;
      return `${downmix2ch}audioconvert ! audioresample ! audio/x-raw,format=S32LE,rate=48000,channels=2` +
        ` ! decklinkaudiosink name=${name} device-number=${dn} sync=true`;
    }

    // ClockStrategy 'ptp-mtl' mit explizitem st2110Audio: primäre Gruppe geht auf
    // mtl_st30p_tx (ST2110-30 PCM-Audio, Intel MTL/E810). sync=true — analog zum
    // decklink-Fall oben: der Sink ist Clock-Provider der Master-Pipeline (Sink-
    // Clock-Auto-Selection, MTL-Kontext ist PTP-locked). dev-port/dev-ip aus
    // sinkCfg.st2110 (vom Aufrufer aus _settings.st2110 befüllt, siehe server.js).
    // ACHTUNG: Element-/Property-Namen nicht gegen echten gst-inspect-1.0-Output
    // verifiziert (MTL bei Implementierung noch nicht installiert) — siehe
    // scripts/install-mtl.sh und den Kommentar bei _buildSt2110VideoSrc in server.js.
    if (sinkCfg.type === 'mtl') {
      const a = sinkCfg.st2110 || {};
      const devProps = `dev-port=${a.devIface || 'auto'}${a.devIp ? ` dev-ip=${a.devIp}` : ''}`;
      return `${downmix2ch}audioconvert ! audioresample ! audio/x-raw,format=S24LE,rate=48000,channels=2` +
        ` ! mtl_st30p_tx name=${name} ${devProps} ip=${a.ip} udp-port=${a.port}` +
  ...
  /**
   * Berechnet eine Routing-Matrix für watchGroup, die Audio aus fallbackGroup
   * bezieht (inklusive Up-/Downmix).
   *
   * Wird für Silence-Fallback verwendet: wenn watchGroup stumm ist, leitet diese
   * Matrix die gleichen Quellkanäle wie fallbackGroup durch watchGroup's Bus,
   * mit automatischem Mix falls die Kanal-Anzahl unterschiedlich ist.
   *
   * @param {AudioGroupConfig} cfg
   * @param {object}           preset           — aktives Preset-Objekt
   * @param {number}           inCh             — Anzahl Eingangskanäle
   * @param {string}           watchGroupId     — stumme Gruppe (Ziel)
   * @param {string}           fallbackGroupId  — aktive Gruppe (Quelle)
   * @param {string}           [mixMethod]      — 'loro'|'ltrt'|'sum' (default: 'loro')
   * @returns {number[][]|null}
   */
  static computeCrossGroupMatrix(cfg, preset, inCh, watchGroupId, fallbackGroupId, mixMethod = 'loro') {
    const watchGrp = cfg.getGroup(watchGroupId);
    const fbGrp    = cfg.getGroup(fallbackGroupId);
    if (!watchGrp || !fbGrp) return null;

    // Routing-Matrix der Fallback-Gruppe aus dem aktuellen Preset
    const fbMat = AudioRouter._matrixFromRoutes(preset.routes, inCh, fallbackGroupId, fbGrp.channels)
      || AudioRouter._identityMatrix(Math.min(inCh, fbGrp.channels), fbGrp.channels);

    // Gleiche Kanal-Anzahl: Matrix direkt übernehmen
    if (watchGrp.channels === fbGrp.channels) return fbMat;

    // Up-/Downmix: fbGrp.channels → watchGrp.channels
    const mixMat = AudioRouter._mixMatrix(mixMethod, fbGrp.channels, watchGrp.channels);
    return AudioRouter._multiplyMatrices(mixMat, fbMat);
  }
