...
class GrafixEngine extends EventEmitter {
   ...
   *   g.delay         — Start in Sekunden nach Clip-Beginn (Fallback wenn kein delayFrames)
   *   g.durationFrames — Dauer in Frames
   *   g.duration      — Dauer in Sekunden (Fallback)
   *   g.endOffset     — Relativer End-Anker: negative Zahl = N Sekunden VOR Clip-Ende
   *                      z.B. -5 → blendet 5s vor Clip-Ende aus
   *   g.endOffsetFrames — Wie endOffset aber in Frames
   *
   * clipDurationSec: Dauer des primären Clips in Sekunden (für relative Endpunkte)
   * fps:             Framerate für Frame→ms Umrechnung
   *
   * @param {object}   event
   * @param {number}   [clipDurationSec]  — Dauer des primären Clips (für endOffset)
   * @param {number}   [fps=25]
   * @param {Function} [onGrafik]         — Callback für UI-Updates
   * @param {object}   [playlistCtx]      — { playlist, currentIndex, fps, classifs } for variable resolution
   * @returns {Function}  cleanup()
   */
  scheduleChildEvents(event, clipDurationSec = null, fps = 25, onGrafik = null, playlistCtx = null) {
    if (!event.children || event.children.length === 0) return () => {};

    const timers = [];
    const activeIds = [];
    const frameDur = 1000 / fps;  // ms pro Frame

    const grafixEvents = event.children.filter(c => c.source === 'grafik' && c.grafik);
    if (!grafixEvents.length) return () => {};

    this._log(`Schedule ${grafixEvents.length} Grafik-Child-Event(s) für "${event.file || event.source}" (clip=${clipDurationSec?.toFixed(2) ?? '?'}s, ${fps}fps)`);
    ...
    for (const child of grafixEvents) {
      ...
      // ── Delay (Start-Zeitpunkt) ─────────────────────────────────────────
      let delayMs;
      if (g.delayFrames != null) {
        delayMs = Math.round(g.delayFrames * frameDur);
      } else {
        delayMs = Math.round((g.delay ?? 0) * 1000);
      }

      // ── Dauer / End-Zeitpunkt ────────────────────────────────────────────
      // Priorität: endOffsetFrames > endOffset (relativ zu Clip-Ende)
      //            > durationFrames > duration (absolut ab Show-Zeitpunkt)
      let durMs = null;

      if (g.endOffsetFrames != null && clipDurationSec != null) {
        // endOffsetFrames: negativ = vor Ende, z.B. -5 = 5 Frames vor Ende
        const clipMs  = clipDurationSec * 1000;
        const endMs   = clipMs + (g.endOffsetFrames * frameDur); // endOffset ist negativ
        durMs = Math.max(0, endMs - delayMs);
      } else if (g.endOffset != null && clipDurationSec != null) {
        const clipMs  = clipDurationSec * 1000;
        const endMs   = clipMs + (g.endOffset * 1000);           // endOffset ist negativ
        durMs = Math.max(0, endMs - delayMs);
      ...
      // null = bis Event-Ende (kein Auto-Hide)

      this._log(
        `  → ${g.template} delay=${(delayMs/1000).toFixed(3)}s` +
      ...
      const showTimer = setTimeout(async () => {
        if (!this._running) return;
        // Resolve playlist variable references in data at show-time (most accurate state)
        const rawData = g.data || {};
        const resolvedData = playlistCtx
          ? this._resolveVars(rawData, {
              ...playlistCtx,
              // Provide elapsed time estimate for starttime calculation
              _scheduleElapsedSec: delayMs / 1000,
            })
          : rawData;
        const shownId = await this.show(g.template, resolvedData, g.layer || 'overlay', id);
    ...
    // Cleanup: alle Timer canceln + aktive Grafiken ausblenden
    // Emits hide for any grafik that was shown but not yet auto-hidden,
    // so the UI's on-air strip is updated regardless of why cleanup runs.
    return () => {
