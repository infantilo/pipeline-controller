...
// oGraf templates load a custom element via dynamic ES-module import inside a
// generated host page (_buildOgrafHostPage) — much slower than a static HTML
// file (_precueFrame's own fallback timeout is 3000ms, i.e. worst-case load
// time). They need a clip-player-like lead time to be fully loaded and
// rendering before their show-timer fires, otherwise they either appear late
// (cold this.show() path) or not at all.
const OGRAF_PRECUE_LEAD_MS = 2500;

// Bild-Children: Browser-Fetch+Decode ist schnell, braucht nur kurze Lead-Time.
const IMAGE_PRECUE_LEAD_MS = 250;
// Mit aktiviertem Chroma-Key (dve.chromaKey) kommt vorher noch GreenZoneDetect.
// chromaKeyToTransparentPng() dazwischen — Dekodieren+Keyen+PNG-Encode über eine
// eigene gst-kit-Pipeline dauert (ungecacht) typischerweise mehrere 100ms bis ~1s.
// Gecachte Treffer (gleicher Pfad+mtime) sind danach quasi instant, aber das erste
// Mal muss die volle Zeit eingeplant sein, sonst erscheint das Bild verzögert
// gegenüber dem (sofortigen) Video-Squeeze.
const IMAGE_CHROMAKEY_PRECUE_LEAD_MS = 1200;

// DVE target='video': Default-Vorlauf, um das Bild (Puppeteer-Render-Rundtrip)
// VOR dem (latenzfreien) Video-Squeeze sichtbar zu machen — siehe scheduleChildEvents
// "doRevealEarly". Wird ab dem ersten tatsächlich gemessenen Reveal pro Engine-Instanz
// adaptiv über this._dveRevealLatencyMs nachgeführt (EMA), dieser Wert ist nur der
// Startwert ~3 Frames @25fps, bis genug Messungen vorliegen.
const DVE_VIDEO_REVEAL_LEAD_MS_DEFAULT = 80;
...
class GrafixEngine extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    this._imageAssets     = new Map();   // grafixId → absoluter Bildpfad (für /grafik/__asset)
    this._fullBackdropOn  = false;       // Backdrop für 'full'-Layer (deckt PGM komplett ab)
  ...
  // ── Layer-Modus ('overlay' | 'full' | 'auto') ─────────────────────────────
  //
  // 'overlay': Grafik wird transparent über das Programmbild gelegt (Standard).
  // 'full':    Grafik soll das Programmbild komplett ersetzen (Vollbild-Takeover,
  //            z.B. Sponsor-Billboard, Technical-Difficulties-Tafel). Da der
  //            Compositor die Grafik-Quelle ohnehin in oberster Zorder einblendet,
  //            genügt es, den Compositor-Hintergrund OPAK zu machen — die Grafik
  //            deckt das PGM-Bild dann vollflächig ab, ohne Pipeline-Änderungen.
  // 'auto':    Nach dem ersten gerenderten Frame wird per Alpha-Channel-Scan
  //            erkannt, ob die Grafik den Frame vollflächig opak füllt → 'full',
  //            sonst → 'overlay'. Ergebnis wird pro Grafik in _resolvedLayer
  //            gecacht (_alphaChecked gated den einmaligen Scan).

  /** Effektiver Layer einer aktiven Grafik (löst 'auto' anhand _resolvedLayer auf). */
  _effectiveLayer(entry) {
    const layer = entry?.layer || 'overlay';
    if (layer === 'auto') return entry._resolvedLayer || 'overlay';
    return layer;
  }

  /** Aktiviert/deaktiviert den opaken Backdrop, je nachdem ob ein 'full'-Layer aktiv ist. */
  _recomputeLayerMode() {
    let anyFull = false;
    for (const entry of this._activeGrafiks.values()) {
      if (this._effectiveLayer(entry) === 'full') { anyFull = true; break; }
    }
    this._setFullBackdrop(anyFull);
  }

  /** Fügt einen vollflächigen opaken <div> hinter den Grafik-Layern ein/entfernt ihn. */
  async _setFullBackdrop(active) {
    if (this._fullBackdropOn === active || !this._page) { this._fullBackdropOn = active; return; }
    this._fullBackdropOn = active;
    try {
      await this._page.evaluate((on) => {
        const ID = '_gfx_full_backdrop';
        let el = document.getElementById(ID);
        if (on) {
          if (!el) {
            el = document.createElement('div');
            el.id = ID;
            el.style.cssText = 'position:absolute;inset:0;background:#000;z-index:-1';
  ...
  /** Scannt den Alpha-Kanal eines RGBA-Buffers — true wenn (nahezu) vollständig opak. */
  _isFullyOpaque(rgba) {
    if (!rgba?.length) return false;
    for (let i = 3; i < rgba.length; i += 4) {
      if (rgba[i] < 250) return false;
    }
    return true;
  }

  /**
   * Wartet auf den nächsten tatsächlich gepushten Compositor-Frame ('frame'-Event,
   * wird nur nach erfolgreichem Screenshot+Decode in _renderFrame() emittiert — siehe
   * dort). Genauer als ein fixes setTimeout(RENDER_INTERVAL): der Puppeteer-Screenshot-
   * Rundtrip (CDP-Roundtrip + PNG-Decode) kann je nach Systemlast länger als ein
   * Render-Intervall dauern, ein fixes Timeout wäre dann ein Race (Squeeze feuert,
   * bevor der neue Frame wirklich draußen ist). Ohne Page (Fallback-Modus) wird
   * nie 'frame' emittiert → Timeout greift, kein Hang.
   * @returns {Promise<boolean>} true wenn ein Frame kam, false bei Timeout
   */
  _waitNextFrame(timeoutMs = 500) {
    if (!this._page) return Promise.resolve(false);
    return new Promise(resolve => {
      const onFrame = () => { clearTimeout(t); resolve(true); };
      const t = setTimeout(() => { this.removeListener('frame', onFrame); resolve(false); }, timeoutMs);
  ...
  async _renderFrame() {
    ...
    try {
      ...
      if (rgba) {
        ...
        // 'auto'-Layer einmalig anhand des ersten gerenderten Frames auflösen:
        // vollflächig opak → 'full' (Vollbild-Takeover), sonst → 'overlay'.
        if (!this._alphaChecked) {
          this._alphaChecked = true;
          const opaque = this._isFullyOpaque(rgba);
          let resolvedAny = false;
          for (const entry of this._activeGrafiks.values()) {
            if (entry.layer === 'auto' && entry._resolvedLayer == null) {
              entry._resolvedLayer = opaque ? 'full' : 'overlay';
              resolvedAny = true;
              this._log(`Auto-Layer "${entry.template}" → ${entry._resolvedLayer} (Frame ${opaque ? 'opak' : 'transparent'})`, 'debug');
            }
          }
          if (resolvedAny) this._recomputeLayerMode();
        }
   ...
   */
  async show(template, data = {}, layer = 'overlay', grafixId = null, box = null) {
    const id = grafixId || `grafik-${Date.now()}`;
    this._log(`Show: ${template} [${layer}] id=${id} (${this.width}×${this.height})`);
    this._activeGrafiks.set(id, { template, data, layer, _resolvedLayer: null });
    this._currentLayer = layer;
    this._alphaChecked = false;
    this._lastFrameHash = null;  // force re-render at full rate when new grafik appears
    // Kick render loop into full-rate mode immediately — without this, _scheduleRender()
    // would keep the stale IDLE interval from before (up to 1s delay until the new
    // content is actually rendered, even though the timer is restarted now).
    this._renderInterval = RENDER_INTERVAL;
    if (this._renderTimer) { clearTimeout(this._renderTimer); this._renderTimer = null; }
    this._scheduleRender();
    this.emit('show', { id, template, data, layer });
    this._recomputeLayerMode();
    if (this._page) {
      ...
      const b = box || { x: 0, y: 0, w: this.width, h: this.height };
      try {
        ...
        // box (DVE/squeeze): Template wird intern weiterhin in voller Pipeline-Auflösung
        // gerendert (Layout/Animationen bleiben unverändert) — das iframe-Element selbst
        // wird per CSS transform:scale() auf box.w×box.h herunterskaliert. transform ist
        // Chromium-GPU-Compositing (Layer-Promotion), kostet keine zusätzliche Rasterzeit.
        await this._page.evaluate((iframeId, iframeSrc, iW, iH, bx) => {
          return new Promise((resolve) => {
            ...
            fr.src  = iframeSrc;
            const sx = bx.w / iW, sy = bx.h / iH;
            fr.style.cssText = `position:absolute;top:0;left:0;width:${iW}px;height:${iH}px;` +
              `border:none;background:transparent;transform-origin:top left;` +
              `transform:translate(${bx.x}px,${bx.y}px) scale(${sx},${sy});`;
        ...
        }, id, url, this.width, this.height, b);
  ...
  /**
   * Zeigt ein statisches Bild (JPG/PNG/GIF/WebP/SVG) als Grafik-Layer an.
   * Im Gegensatz zu show() wird kein Template-iframe geladen, sondern ein
   * <img> direkt im Compositor-DOM gerendert; die Datei wird über die
   * interne /grafik/__asset Route ausgeliefert (per grafixId, kein Pfad in der URL).
   * @param {string} absPath — absoluter Pfad zur Bilddatei
   * @param {string} [layer='overlay'] — 'overlay' | 'full' | 'auto'
   * @param {string} [grafixId]
   * @returns {string} grafixId
   */
  async showImage(absPath, layer = 'overlay', grafixId = null, box = null) {
    const id    = grafixId || `grafik-img-${Date.now()}`;
    const label = path.basename(absPath);
    this._log(`Show Image: ${label} [${layer}] id=${id} (${this.width}×${this.height})`);
    this._imageAssets.set(id, absPath);
    this._activeGrafiks.set(id, { template: label, data: {}, layer, isImage: true, _resolvedLayer: null });
    this._currentLayer = layer;
    this._alphaChecked = false;
    this._lastFrameHash = null;
    this._renderInterval = RENDER_INTERVAL;  // see show() — avoid stale IDLE-rate delay
    if (this._renderTimer) { clearTimeout(this._renderTimer); this._renderTimer = null; }
    this._scheduleRender();
    this.emit('show', { id, template: label, data: {}, layer });
    this._recomputeLayerMode();
    if (this._page) {
      const src = `/grafik/__asset?id=${encodeURIComponent(id)}&t=${Date.now()}`;
      // box (DVE/squeeze): {x,y,w,h} in px, fit: 'contain'|'cover'|'fill' — default = full frame.
      const b   = box || { x: 0, y: 0, w: this.width, h: this.height, fit: 'contain' };
      const fit = b.fit === 'fill' ? 'fill' : (b.fit || 'contain');
      try {
        await this._page.evaluate((elId, imgSrc, bx, objFit) => {
          return new Promise((resolve) => {
            ...
            img.name = elId;
            img.src = imgSrc;
            img.style.cssText = `position:absolute;left:${bx.x}px;top:${bx.y}px;width:${bx.w}px;height:${bx.h}px;` +
              `object-fit:${objFit};background:transparent;`;
            const fallback = setTimeout(resolve, 3000);
            img.addEventListener('load',  () => { clearTimeout(fallback); setTimeout(resolve, 80); }, { once: true });
            img.addEventListener('error', () => { clearTimeout(fallback); resolve(); }, { once: true });
            document.body.appendChild(img);
          });
        }, id, src, b, fit);
      } catch(e) { this._log(`Image-Load: ${e.message}`, 'warn'); }
    }
    return id;
  }
  ...
  async hide(grafixId) {
    ...
    this._imageAssets.delete(grafixId);
   ...
   */
  async _precueFrame(id, url, W, H, box = null) {
    ...
    try {
      await this._page.evaluate((iframeId, iframeSrc, iW, iH, bx) => {
        return new Promise(resolve => {
          ...
          fr.style.cssText = `position:absolute;top:0;left:0;width:${iW}px;height:${iH}px;border:none;` +
            `background:transparent;visibility:hidden;transform-origin:top left;` +
            `transform:translate(${bx.x}px,${bx.y}px) scale(${sx},${sy});`;
          fr.allowTransparency = true;
          const fallback = setTimeout(resolve, 3000);
          fr.addEventListener('load', () => { clearTimeout(fallback); setTimeout(resolve, 80); }, { once: true });
  ...
  /**
   * Injiziert ein Bild hidden ins Compositor-DOM (Pendant zu _precueFrame, für
   * source:'image'-Children). Lädt die Datei im Browser vor UND — falls vorher
   * via Chroma-Key verarbeitet — wartet den vollständigen Decode/Encode-Vorgang
   * ab, BEVOR der Show-Zeitpunkt erreicht ist. Ohne dieses Pre-Cue müsste der
   * komplette Bild-Pfad (ggf. Chroma-Key: dekodieren → keyen → PNG encodieren →
   * Browser-Fetch → Decode) erst im Show-Timer laufen — das verzögerte bei
   * target='video' sichtbar das Bild gegenüber dem (sofort wirksamen) Video-
   * Squeeze um bis zu ~1s.
   * @returns {boolean} true wenn erfolgreich vorgeladen
   */
  async _precueImage(id, absPath, box = null) {
    if (!this._page || this._pageRecreating) return false;
    this._imageAssets.set(id, absPath);
    const b = box || { x: 0, y: 0, w: this.width, h: this.height, fit: 'contain' };
    const fit = b.fit === 'fill' ? 'fill' : (b.fit || 'contain');
    ...
    try {
      await this._page.evaluate((elId, imgSrc, bx, objFit) => {
        return new Promise(resolve => {
          ...
          if (old) old.remove();
          const img = document.createElement('img');
          img.id = elId; img.name = elId;
          img.src = imgSrc;
          img.style.cssText = `position:absolute;left:${bx.x}px;top:${bx.y}px;width:${bx.w}px;height:${bx.h}px;` +
            `object-fit:${objFit};background:transparent;visibility:hidden;`;
          const fallback = setTimeout(resolve, 3000);
      ...
      this._log(`_precueImage ${id}: ${e.message}`, 'debug');
      return false;
    }
  }

  /**
   * Macht ein per _precueImage vorgeladenes Bild sichtbar und positioniert es final
   * (Box kann abweichen, falls inzwischen eine Zone neu aufgelöst wurde — CSS-Update
   * ist im Gegensatz zum Bild-Laden selbst praktisch kostenlos/instant).
   * @returns {boolean} true wenn Element gefunden und sichtbar gemacht
   */
  async _showPrecuedImage(id, layer, label, box = null) {
    if (!this._page) return false;
    const b = box || { x: 0, y: 0, w: this.width, h: this.height, fit: 'contain' };
    const fit = b.fit === 'fill' ? 'fill' : (b.fit || 'contain');
    try {
      const found = await this._page.evaluate((elId, bx, objFit) => {
        const el = document.getElementById(elId);
        if (!el) return false;
        el.style.left = bx.x + 'px'; el.style.top = bx.y + 'px';
        el.style.width = bx.w + 'px'; el.style.height = bx.h + 'px';
      ...
      this._log(`_showPrecuedImage ${id}: ${e.message}`, 'debug');
      return false;
    }
  }
  ...
  /**
   * DVE/Squeeze — statische Box aus child.dve (normalisiert 0..1) in px auflösen.
   * Kein Rotation-Support (per Anforderung: GPU-frei skalierbar, keine CPU-Rotation).
   * @param {object} dve  — { x, y, w, h, fit } alle 0..1, fit default 'contain'
   */
  _dveBoxPx(dve, W, H) {
    if (!dve) return { x: 0, y: 0, w: W, h: H, fit: 'contain' };
    return {
      x: (dve.x ?? 0) * W,
      y: (dve.y ?? 0) * H,
      w: (dve.w ?? 1) * W,
  ...
  /**
   * DVE-Box aus einer "Zone" eines gleichzeitig laufenden Grafik-Siblings lesen.
   * Konvention (kein oGraf-Standard dafür vorhanden — selbst definiert):
   *   Ein beliebiges Element im Template (Light-DOM oder Custom-Element-Shadow-DOM)
   *   trägt `data-dve-zone="<name>"`. getBoundingClientRect() liefert die Box in
   *   px relativ zum Template-Viewport (= W×H, da das Template stets in voller
   *   Auflösung rendert — siehe show()/transform-Skalierung).
   * @returns {Promise<{x,y,w,h}|null>}
   */
  async _resolveDveZone(zoneName, siblingGenId) {
    if (!this._page || !zoneName || !siblingGenId) return null;
    const frame = this._page.frames().find(f => f.name() === siblingGenId);
    if (!frame) return null;
    try {
      return await frame.evaluate((zone) => {
        const findIn = (root) => {
          if (!root || !root.querySelectorAll) return null;
          for (const c of root.querySelectorAll('[data-dve-zone]')) {
            if (c.getAttribute('data-dve-zone') === zone) return c;
          }
          return null;
        };
        let el = findIn(document);
        if (!el) {
          for (const host of document.querySelectorAll('*')) {
            if (host.shadowRoot) { el = findIn(host.shadowRoot); if (el) break; }
          }
        }
        if (!el) return null;
      ...
      this._log(`_resolveDveZone "${zoneName}": ${e.message}`, 'debug');
      return null;
    }
  }

  /**
   * DVE target='video', Child startet AB CLIP-BEGINN (delay≈0): muss VOR dem
   * eigentlichen Cut/Switch (MasterPipeline.switchTo) angewendet werden, sonst
   * läuft das erste Frame des neuen Clips noch unkomprimiert raus — der normale
   * scheduleChildEvents()-Pfad feuert showTimer/Squeeze immer NACH dem Switch
   * (der Aufruf kommt von dort), egal wie kurz effDelayMs ist. Wird vom Aufrufer
   * (PlaylistEngine._preApplyImmediateDve) SYNCHRON vor switchFn() awaited.
   * Markiert das Child-Objekt selbst (child._dvePreApplied = id) — scheduleChildEvents()
   * erkennt das und überspringt den Re-Show, übernimmt aber Hide-Timer/Cleanup normal.
   * @returns {Promise<{id:string, box:{x,y,w,h}}|null>}
   */
  async prepareImmediateDveSqueeze(child, opts = {}) {
    if (!this._page || this._pageRecreating || child.source !== 'image' || !child.src) return null;
    const absImagePath = (typeof opts.resolveImagePath === 'function') ? opts.resolveImagePath(child.src) : null;
    if (!absImagePath) return null;
    let imgPath = absImagePath;
    if (child.dve?.chromaKey) {
      try {
        imgPath = await require('./GreenZoneDetect').chromaKeyToTransparentPng(absImagePath);
      } catch(e) {
        this._log(`Chroma-Key "${child.src}": ${e.message} — Originalbild ohne Transparenz`, 'warn');
      }
    }
    const id = `child-preapplied-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
    const loaded = await this._precueImage(id, imgPath, null);
    if (!loaded) return null;
    const shown = await this._showPrecuedImage(id, child.layer || 'overlay', child.src, null);
    if (!shown) return null;
    child._dvePreApplied = id;
    return { id, box: this._dveBoxPx(child.dve, this.width, this.height) };
  }
  ...
  scheduleChildEvents(event, clipDurationSec = null, fps = 25, onGrafik = null, playlistCtx = null, opts = {}) {
    ...
    const grafixEvents = event.children.filter(c =>
      (c.source === 'grafik' && c.grafik) || (c.source === 'image' && c.src));
    ...
    // Ids aller Siblings vorab vergeben (statt erst im Loop) — DVE-Zone-Referenzen
    // (child.dve.zoneFrom) müssen auf Siblings zeigen können, bevor diese selbst
    // an die Reihe kommen (z.B. Bild-Squeeze + oGraf-Grafik gleichzeitig im Event).
    const idByChild = new Map();
    // Bereits per prepareImmediateDveSqueeze() (VOR dem Clip-Switch) angewendete Children
    // behalten ihre dort vergebene id — sonst würde _activeGrafiks unter zwei verschiedenen
    // ids für dasselbe sichtbare Element geführt (Hide-Cleanup würde dann ins Leere laufen).
    for (const c of grafixEvents) idByChild.set(c, c._dvePreApplied || `child-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`);
    // id → war dieses Child DVE-aktiv? (für Cleanup-Closure unten, die nur noch
    // mit reinen id-Strings arbeitet — siehe activeIds)
    const dveActiveById = new Map();

    for (const child of grafixEvents) {
      // Bild-Children tragen ihre Felder direkt am Child (siehe _mapGrafikChild),
      // HTML/oGraf-Children kapseln sie in .grafik — auf gemeinsame Form `g` normalisieren.
      const isImage = child.source === 'image';
      const g = isImage ? child : child.grafik;
      ...
      const id = idByChild.get(child);
      activeIds.push(id);
      dveActiveById.set(id, g.dve?.target === 'video');
      ...
      this._log(
        `  → ${isImage ? g.src : g.template} nativeDelay=${(nativeDelayMs/1000).toFixed(3)}s effDelay=${(effDelayMs/1000).toFixed(3)}s` +
      ...
      const W = this.width, H = this.height;
      const isOgraf = !isImage && this._resolveTemplate(g.template)?.type === 'ograf';
      let _precued = false;
      let templateUrl = null;
      let absImagePath = null;
      // dveTarget='video' Bild-Children: vorab (vor effDelayMs) sichtbar gemachtes Bild —
      // siehe "doRevealEarly" weiter unten. Show-Timer nutzt das, statt erneut zu zeigen.
      let _dveRevealed = false;
      let _dveRevealedShownId = null;

      // ── DVE/Squeeze: Position & Größe dieses Children (g.dve) ────────────
      // g.dve = { x,y,w,h (0..1, normalisiert), fit:'contain'|'cover'|'fill' }
      // g.dve.zone + g.dve.zoneFrom: Box stattdessen aus [data-dve-zone] eines
      // gleichzeitig laufenden Sibling-Templates lesen (siehe _resolveDveZone).
      // g.dve.target: 'image' (default) = dieses Child wird gesqueezt+positioniert,
      //               Programm-/Live-Video bleibt Vollbild (Bild/Grafik als kleines
      //               Overlay über dem Video).
      //               'video' = das Programm-/Live-Video wird in die Box gesqueezt,
      //               dieses Child wird stattdessen VOLLBILD gezeigt (z.B. ein
      //               Rahmen-Grafik/Bild mit transparentem Loch an der Box-Stelle —
      //               siehe g.dve.chromaKey) und liegt als Rahmen ÜBER dem gesqueezten
      //               Video (Compositor-Zorder: Video sink_0 < Grafik sink_2).
      // g.dve.chromaKey: nur bei isImage relevant — macht grüne Pixel im Bild vor der
      //               Anzeige transparent (GreenZoneDetect.chromaKeyToTransparentPng),
      //               damit das gesqueezte Video durch das "Loch" sichtbar wird.
      // Keine Rotation — nur GPU-billige Translate/Scale (kein CPU-Rotationspfad).
      const dve = g.dve || null;
      const dveTarget = dve?.target === 'video' ? 'video' : 'image';
      const dveStaticBox = dve ? this._dveBoxPx(dve, W, H) : null;
      // selfStaticBox: Box für die EIGENE Darstellung dieses Children (null = Vollbild,
      // wenn target='video' — die Box gilt dann nur für das Video-Squeeze, siehe unten).
      const selfStaticBox = dveTarget === 'video' ? null : dveStaticBox;
      let dveZoneSiblingId = null;
      if (dve?.zone) {
        const sibling = dve.zoneFrom != null
          ? grafixEvents.find(c => c !== child && String(c.id) === String(dve.zoneFrom))
          : grafixEvents.find(c => c !== child && c.source === 'grafik');
        dveZoneSiblingId = sibling ? idByChild.get(sibling) : null;
        if (!dveZoneSiblingId) this._log(`DVE-Zone "${dve.zone}": kein Sibling-Template gefunden (zoneFrom=${dve.zoneFrom ?? '(auto)'})`, 'warn');
      }
      // Zone-Lookup braucht das gerenderte Sibling-DOM → kein Pre-Cue (Box erst zur
      // Show-Zeit bekannt). Statische Boxen (kein zone) können weiterhin pre-gecuet werden.
      const skipPrecueForZone = !isImage && !!dve?.zone;

      // imgPathForPrecue: vom Pre-Cue ggf. bereits chroma-key-verarbeiteter Pfad —
      // im Show-Timer wiederverwendet statt erneut zu verarbeiten.
      let imgPathForPrecue = null;

      // Bereits vor dem Clip-Switch angewendet (prepareImmediateDveSqueeze, siehe
      // PlaylistEngine._preApplyImmediateDve) — Bild UND Squeeze sind schon live,
      // bevor der erste Frame des neuen Clips überhaupt durch die Pipeline läuft.
      // Hier nur noch als "bereits sichtbar" markieren, kein erneutes Precue/Reveal —
      // das würde den gerade erst erreichten Frame-genauen Sync wieder zerstören.
      const isPreApplied = isImage && id === child._dvePreApplied;
      if (isPreApplied) {
        _precued = true;
        _dveRevealed = true;
        _dveRevealedShownId = id;
        delete child._dvePreApplied;  // einmalig — nächster Play dieses Events ruft erneut auf
      } else if (isImage) {
        // Bild-Children werden direkt per <img> gerendert (kein Template-iframe).
        // Pre-Cue (hidden, vorab geladen) ist trotzdem nötig: ohne Chroma-Key ist das
        // reine Browser-Fetch+Decode meist schnell genug, MIT Chroma-Key (Dekodieren+
        // Keyen+PNG-Encode über gst-kit) kann es ungecacht ~1s dauern — sichtbar als
        // Verzögerung zwischen dem (sofortigen) Video-Squeeze und dem Bild selbst.
        absImagePath = (typeof opts.resolveImagePath === 'function') ? opts.resolveImagePath(g.src) : null;
        if (!absImagePath) {
          this._log(`Bild-Child "${g.src}" nicht gefunden (gesucht in images/, channelbranding/, media/) — übersprungen`, 'warn');
          continue;
        }

        const precueLeadMs = dve?.chromaKey ? IMAGE_CHROMAKEY_PRECUE_LEAD_MS : IMAGE_PRECUE_LEAD_MS;
        const precueFireMs = effDelayMs > precueLeadMs ? effDelayMs - precueLeadMs : 0;
        const doPrecueImage = async () => {
          let imgPath = absImagePath;
          if (dve?.chromaKey) {
            try {
              imgPath = await require('./GreenZoneDetect').chromaKeyToTransparentPng(absImagePath);
            } catch(e) {
              this._log(`Chroma-Key "${g.src}": ${e.message} — Originalbild ohne Transparenz`, 'warn');
            }
          }
          imgPathForPrecue = imgPath;
          _precued = await this._precueImage(id, imgPath, selfStaticBox);
        };
        if (precueFireMs > 0) {
          const precueTimer = setTimeout(() => { if (this._running) doPrecueImage(); }, precueFireMs);
          timers.push(precueTimer);
        } else if (this._page && !this._pageRecreating) {
          doPrecueImage();
        }

        // ── DVE target='video': Bild VOR dem Squeeze sichtbar machen ────────
        // Das Sichtbarmachen (_showPrecuedImage) selbst hat Latenz (Puppeteer-
        // Screenshot-Rundtrip bis der Frame tatsächlich beim Compositor ankommt —
        // siehe _waitNextFrame). setDveSqueeze() ist dagegen eine reine GStreamer-
        // Property, praktisch ohne Latenz. Wenn beide am selben effDelayMs-Zeitpunkt
        // ausgelöst werden, läuft der Squeeze dem sichtbaren Bild IMMER ein Stück
        // voraus. Lösung: Bild schon "revealLeadMs" VOR effDelayMs sichtbar machen
        // (Lead = letzte gemessene Reveal-Dauer, adaptiv über _dveRevealLatencyMs),
        // squeeze unverändert exakt bei effDelayMs feuern (siehe Show-Timer unten) —
        // dann ist das Bild zum Squeeze-Zeitpunkt schon durch den Render-Loop draußen.
        if (dveTarget === 'video') {
          const revealLeadMs = this._dveRevealLatencyMs ?? DVE_VIDEO_REVEAL_LEAD_MS_DEFAULT;
          const revealFireMs = effDelayMs > revealLeadMs ? effDelayMs - revealLeadMs : 0;
          const doRevealEarly = async () => {
            if (!this._running || !_precued) return;  // noch nicht gecuet → Show-Timer-Fallback greift
            const t0 = Date.now();
            const ok = await this._showPrecuedImage(id, g.layer || 'overlay', g.src, selfStaticBox);
            if (!ok) return;
            // Flag SOFORT setzen — das Bild ist ab hier im DOM sichtbar (page.evaluate ist
            // bereits zurück). Der folgende _waitNextFrame()-Wait dient NUR der Kalibrierung
            // (EMA) und darf das Setzen des Flags nicht blockieren — sonst sieht der
            // Show-Timer (der exakt bei effDelayMs feuert) _dveRevealed evtl. noch als false,
            // zeigt das Bild ein zweites Mal und reißt die ganze Synchronisierung wieder auf.
            _dveRevealed = true;
            _dveRevealedShownId = id;
            const got = await this._waitNextFrame();
            if (got) {
              // EMA: nächstes Mal etwas mehr/weniger Vorlauf nehmen, basierend auf
              // tatsächlich gemessener Dauer bis der Frame draußen war.
              const actualMs = Date.now() - t0;
              this._dveRevealLatencyMs = Math.round(
                ((this._dveRevealLatencyMs ?? DVE_VIDEO_REVEAL_LEAD_MS_DEFAULT) * 0.7) + (actualMs * 0.3)
              );
            }
          };
          const revealTimer = setTimeout(() => { if (this._running) doRevealEarly(); }, revealFireMs);
        ...
        templateUrl = `http://localhost:${this.port}/grafik/${encodeURIComponent(g.template)}?${params}`;

        // oGraf templates (custom element via dynamic host page) load much slower
        // than static HTML — give them a clip-player-like lead time so they're
        // fully rendered before the show-timer fires (see OGRAF_PRECUE_LEAD_MS).
        const precueLeadMs = isOgraf ? OGRAF_PRECUE_LEAD_MS : GRAFIK_PRECUE_LEAD_MS;
        // If effDelayMs is shorter than the desired lead, start pre-cueing right
        // away (fire-and-forget below) instead of scheduling a timer with 0 delay —
        // this gives the template the full effDelayMs window to load.
        const precueFireMs = effDelayMs > precueLeadMs ? effDelayMs - precueLeadMs : 0;

        if (!skipPrecueForZone) {
          if (precueFireMs > 0) {
            const precueTimer = setTimeout(async () => {
              if (!this._running) return;
              _precued = await this._precueFrame(id, templateUrl, W, H, selfStaticBox);
            }, precueFireMs);
            timers.push(precueTimer);
          } else if (this._page && !this._pageRecreating) {
            // Sofort pre-cuen (zu wenig Vorlaufzeit → async, fire-and-forget)
            this._precueFrame(id, templateUrl, W, H, selfStaticBox).then(ok => { _precued = ok; });
          }
        }
      }
      ...
      const showTimer = setTimeout(async () => {
        ...
        // DVE-Box final auflösen: bei zone-Bindung jetzt das Sibling-DOM abfragen
        // (muss bereits gerendert sein — siehe skipPrecueForZone), sonst statische Box.
        // resolvedBox gilt als Squeeze-Ziel für Video (target='video') UND/ODER als
        // eigene Darstellungsbox (target='image', Default).
        let resolvedBox = dveStaticBox;
        if (dve?.zone) {
          const resolved = await this._resolveDveZone(dve.zone, dveZoneSiblingId);
          resolvedBox = resolved ? { ...resolved, fit: dve.fit || 'contain' } : dveStaticBox;
          if (!resolved) this._log(`DVE-Zone "${dve.zone}" nicht im Sibling-DOM gefunden — Fallback auf statische Box`, 'debug');
        }
        // selfBox: Box für die EIGENE Anzeige dieses Children — null (Vollbild)
        // wenn target='video' (dann gilt resolvedBox stattdessen für das Video-Squeeze).
        const selfBox = dveTarget === 'video' ? null : resolvedBox;

        if (isImage) {
          if (_dveRevealed) {
            // dveTarget='video': Bild wurde bereits vor effDelayMs sichtbar gemacht
            // (siehe "doRevealEarly") — hier NICHT erneut zeigen, sonst würde der
            // Vorlauf durch einen zweiten (jetzt zu späten) Render-Rundtrip zunichte
            // gemacht. shownId direkt übernehmen, Squeeze unten feuert sofort.
            shownId = _dveRevealedShownId;
          } else if (_precued) {
            // Bereits vorgeladen (inkl. ggf. Chroma-Key) → nur sichtbar machen + final
            // positionieren (frame-genau, ohne erneute Decode/Encode-Latenz).
            const ok = await this._showPrecuedImage(id, layer, g.src, selfBox);
            shownId = ok ? id : await this.showImage(imgPathForPrecue || absImagePath, layer, id, selfBox);
          } else {
            // Pre-Cue nicht (rechtzeitig) gelungen — Fallback: Chroma-Key + Anzeige jetzt,
            // mit der bekannten Verzögerung (besser spät sichtbar als nie).
            let imgPath = absImagePath;
            if (dve?.chromaKey) {
              try {
                imgPath = await require('./GreenZoneDetect').chromaKeyToTransparentPng(absImagePath);
            ...
            shownId = await this.showImage(imgPath, layer, id, selfBox);
          }
        } else {
          ...
          if (_precued) {
            ...
            // Pre-cue URL trägt rohes g.data (Vars noch nicht aufgelöst):
            //   • oGraf: grafixUpdate() drückt resolvedData via __OGRAF_HOST__.update()
            //   • HTML: kein update()-Kanal → fall back auf show() wenn Vars aufgelöst wurden
            const needsReload = !isOgraf && JSON.stringify(rawData) !== JSON.stringify(resolvedData);
            const ok = !needsReload && await this._showPrecued(id, g.template, resolvedData, layer);
            if (ok && isOgraf) await this.grafixUpdate(id, resolvedData).catch(() => {});
            shownId = ok ? id : await this.show(g.template, resolvedData, layer, id, selfBox);
          } else {
            shownId = await this.show(g.template, resolvedData, layer, id, selfBox);
          }
        }
        const childLabel = isImage ? g.src : g.template;
        // dve: px-Box (Squeeze-Ziel) NUR wenn target='video' — Caller (PlaylistEngine)
        // squeezt damit das eigentliche Programm-/Live-Video (MasterPipeline.setDveSqueeze).
        // Bei isImage+dveTarget='video' ist das Bild dank "doRevealEarly" oben bereits
        // sichtbar — Squeeze darf hier also wieder SOFORT feuern (kein await mehr nötig).
        // (HTML/oGraf-Children mit dve.target='video' haben dieses Pre-Reveal noch nicht —
        // dort kann ein kleiner Squeeze-Vorsprung von <1 Frame bleiben.)
        if (onGrafik) onGrafik({ action: 'show', id: shownId, template: childLabel, dve: dveTarget === 'video' ? resolvedBox : null });

        if (durMs !== null && durMs > 0) {
          // Symmetrisch zum Reveal: bei dveTarget='video' das Bild "revealLeadMs" VOR dem
          // eigentlichen Hide-Zeitpunkt ausblenden, den Squeeze aber erst zum ORIGINAL
          // geplanten Zeitpunkt zurücknehmen — das Bild ist dann (durch den Render-Loop
          // bereits transparent geworden) rechtzeitig weg, bevor das Video wieder vollflächig wird.
          const revealLeadMs = (isImage && dveTarget === 'video')
            ? (this._dveRevealLatencyMs ?? DVE_VIDEO_REVEAL_LEAD_MS_DEFAULT) : 0;
          const imageHideFireMs = Math.max(0, durMs - revealLeadMs);
          const hideTimer = setTimeout(async () => {
            await this.hide(id);
            if (revealLeadMs === 0 && onGrafik) onGrafik({ action: 'hide', id, template: childLabel, dve: dveTarget === 'video' });
          }, imageHideFireMs);
          timers.push(hideTimer);
          if (revealLeadMs > 0) {
            const squeezeClearTimer = setTimeout(() => {
              if (onGrafik) onGrafik({ action: 'hide', id, template: childLabel, dve: true });
            }, durMs);
            timers.push(squeezeClearTimer);
          }
    ...
    return () => {
      ...
      for (const id of activeIds) {
        ...
        const isDve = !!dveActiveById.get(id);
        const entry = this._activeGrafiks.get(id);
        if (entry) {
          this._activeGrafiks.delete(id);
          this._imageAssets.delete(id);
          hidAny = true;
          if (onGrafik) onGrafik({ action: 'hide', id, template: entry.template, dve: isDve });
        } else if (isDve) {
          // Eintrag wurde bereits durch den imageHideTimer entfernt (revealLeadMs-Fenster),
          // der squeezeClearTimer wurde aber gerade gecancelt — DVE-Hide trotzdem feuern.
          if (onGrafik) onGrafik({ action: 'hide', id, template: '', dve: true });
        }
        // Auch pre-gecuete (noch nicht angezeigte) iframes/Bilder entfernen
      ...
      if (hidAny) this._recomputeLayerMode();
  ...
  _startHttpServer() {
    this._server = http.createServer((req, res) => {
      ...
      // Liefert die Bilddatei eines aktiven Bild-Child-Events aus (siehe showImage()).
      // Auflösung über grafixId statt Dateipfad in der URL → kein Path-Traversal-Risiko.
      if (pth === '/grafik/__asset') {
        const assetId  = url.searchParams.get('id');
        const absPath  = assetId ? this._imageAssets.get(assetId) : null;
        if (!absPath || !fs.existsSync(absPath)) { res.writeHead(404); res.end(); return; }
