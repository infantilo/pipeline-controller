...
class GrafixEngine extends EventEmitter {
   ...
   * @returns {string} grafixId
   */
  async showImage(absPath, layer = 'overlay', grafixId = null) {
    ...
    if (this._page) {
      ...
      try {
        await this._page.evaluate((elId, imgSrc, iW, iH) => {
          return new Promise((resolve) => {
            ...
            img.name = elId;
            img.src = imgSrc;
            img.style.cssText = `position:absolute;top:0;left:0;width:${iW}px;height:${iH}px;` +
  ...
  scheduleChildEvents(event, clipDurationSec = null, fps = 25, onGrafik = null, playlistCtx = null, opts = {}) {
    ...
    for (const child of grafixEvents) {
      ...
      if (isImage) {
        ...
        // kein Pre-Cue nötig — das Laden der Datei aus dem Asset-Cache ist schnell).
      ...
      const showTimer = setTimeout(async () => {
        ...
        if (isImage) {
          shownId = await this.showImage(absImagePath, layer, id);
        } else {
        ...
        if (onGrafik) onGrafik({ action: 'show', id: shownId, template: childLabel });

        if (durMs !== null && durMs > 0) {
          const hideTimer = setTimeout(async () => {
            await this.hide(id);
            if (onGrafik) onGrafik({ action: 'hide', id, template: childLabel });
          }, durMs);
