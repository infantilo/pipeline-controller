...
class PlaylistEngine extends EventEmitter {
  ...
  async _doSwapOnAirVariant({ audioPreset, afd } = {}) {
    ...
    const altEvent = {
      ...
      audioConfig: { ...(curEv.audioConfig ?? {}), preset: newPreset },
    };
    ...
    if (!altPlayer.cued) { this._log(`On-Air-Wechsel: Pre-Cue auf ${altSlot} fehlgeschlagen`, 'warn'); return false; }
  ...
  _armNegChildSchedule(event, slot, leadMs) {
    ...
    if (!this.grafixEngine || !_hasNegDelay) return;
