...
class PlaylistEngine extends EventEmitter {
  ...
  validate() {
    return this.playlist.map(ev => {
      // Explizite DeckLink-Live-Quelle (source = live1/live2/…): Defekt oder fehlendes
      // Eingangssignal wird wie eine fehlende Datei behandelt (gleicher ok:false-Pfad,
      // damit die UI dieselbe "Missing"-Pille zeigt wie bei einem fehlenden Clip).
      // Generische Live-Events (source='live', Auto-Slot-Zuweisung) bleiben hier
      // außen vor — ihre tatsächliche Quelle steht erst zur Abspielzeit fest
      // (_liveSlotFor()), ist also vorab nicht statisch prüfbar.
      if (this._isLiveSource(ev)) {
        const lsConf = this._liveSources.find(ls => ls.id === ev.source);
        if (lsConf && /decklink/i.test(lsConf.gstSrc || '')) {
          if (this.master?.isLiveSourceBroken?.(ev.source)) {
            return { id: ev.id, ok: false, error: 'DeckLink: Defekt — Platzhalter aktiv' };
          }
          if (this.master?.getLiveSignalStatus?.(ev.source) === false) {
            return { id: ev.id, ok: false, error: 'DeckLink: Kein Signal' };
          }
        }
