...
class PlaylistEngine extends EventEmitter {
  ...
  async startCueFirst(fromIndex = 0) {
    ...
    if (precuedSlot) {
      try {
        // Während Pre-Roll auf Schwarz wechseln → verhindert SMPTE-Ton-Durchgang
  ...
  _cancelChildEvents() {
    ...
    // Negative-delay pre-child-cleanups (aus _doCue) ebenfalls abbrechen
    for (const fn of this._preChildCleanups.values()) {
      try { fn(); } catch {}
    }
    this._preChildCleanups.clear();
  ...
  async _executeEvent(event) {
    ...
    if (src === 'block_start') {
      ...
      if (this.grafixEngine && event.children?.length) {
        ...
        this._blockChildCleanup = this.grafixEngine.scheduleChildEvents(
          ...
          _plCtxBlock,
          { latencyMs: this.opts.grafikLatencyMs || 0 }
        );
      }
      this._setState(this.currentIndex, 'done');
      this._advancing = false;
    ...
    if (this.grafixEngine && event.children?.length) {
      ...
      }, _plCtx, { latencyMs: this.opts.grafikLatencyMs || 0 });
  ...
  async _doCue(event, slot, absFile, somSec, eomAbs) {
    ...
    // Pre-schedule negative-delay grafik children (delay < 0 = sollen VOR Clip-Start erscheinen).
    // _doCue wird PRE_CUE_MS vor dem Clip-Start aufgerufen → preStartMs = PRE_CUE_MS.
    const _hasNegDelay = (event.children || []).some(c =>
      c.source === 'grafik' && c.grafik &&
      (c.grafik.delayFrames != null ? c.grafik.delayFrames < 0 : (c.grafik.delay ?? 0) < 0));
    if (this.grafixEngine && _hasNegDelay) {
      // Cleanup für vorherigen pre-schedule dieses Slots
      ...
      const preCleanup = this.grafixEngine.scheduleChildEvents(
        ...
        { latencyMs: this.opts.grafikLatencyMs || 0, preStartMs: PRE_CUE_MS, negativeOnly: true }
      );
      this._preChildCleanups.set(slot, preCleanup);
    }
  }
