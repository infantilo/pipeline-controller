...
// Graveyard: holds pipeline references until gst_element_set_state(NULL) confirms.
// Prevents the JS GC from dropping the C++ unique_ptr while GStreamer is still
// tearing down the pipeline (which would trigger "element in PLAYING state" warnings).
const _stopping = new Set();

// `p.stop(-1)` wartet UNBEGRENZT (-1 = GST_CLOCK_TIME_NONE) auf den
// Statewechsel nach NULL — das kann bei manchen Element-Kombinationen
// (z.B. interaudiosink/-src, mxfdemux-Threads) hängen bleiben. Reines
// fire-and-forget vermeidet zwar das Hängen, öffnet aber ein Zeitfenster,
// in dem alte UND neue Pipeline denselben "interaudiosink channel=…" (ein
// gemeinsamer, statischer Inter-Pipeline-Ringpuffer) gleichzeitig
// beanspruchen → Ringpuffer-Korruption → Master-seitiger interaudiosrc
// blockiert dauerhaft → ALLE Audioausgänge stumm/hängen.
//
// Lösung: BEGRENZT warten (deckt den Normalfall ab, schließt das
// Race-Fenster), aber nach `waitMs` trotzdem weitermachen — der
// Stop-Vorgang läuft im Hintergrund weiter (Graveyard-Set hält die
// Referenz am Leben, bis er irgendwann selbst durchläuft).
async function _stopPipeline(p, waitMs = 1500) {
  if (!p) return;
  _stopping.add(p);
  const done = (async () => { try { await p.stop(-1); } catch {} finally { _stopping.delete(p); } })();
...
class PlayerPipeline extends EventEmitter {
  ...
  async load(item) {
    ...
    this._lastPStr = vStr;
    this._log('Lade Video-Pipeline', 'info');
    const vPipeline = await this._tryPipeline(vStr, 12000);

    if (!vPipeline) {
      const errMsg = 'Video-Pipeline fehlgeschlagen';
      this._log(errMsg, 'error');
      if (this.opts.debugger?.enabled) this.opts.debugger.dumpOnError(this.slotId, vStr, errMsg);
  ...
  _videoPath(ch, fps) {
    ...
    switch (afd) {
      case '4:3-pillarbox':
        ...
        afdEl = `! videoconvert ! aspectratiocrop aspect-ratio=4/3` +
      ...
      case '4:3-letterbox':
        ...
        afdEl = `! videoconvert ! aspectratiocrop aspect-ratio=${W}/${H}` +
                ` ! videoscale method=${m} ! video/x-raw,width=${W},height=${H},pixel-aspect-ratio=1/1`;
        break;
      case '14:9-letterbox':
        // 14:9-Balken entfernen → auf Pipelineformat (kleine Balken bleiben im Master)
        afdEl = `! videoconvert ! aspectratiocrop aspect-ratio=14/9` +
      ...
      case 'anamorphic':
        ...
        afdEl = `! videoscale method=${m} ! videoconvert ! video/x-raw,pixel-aspect-ratio=1/1` +
                ` ! videoscale method=${m} ! video/x-raw,width=${W},height=${H},pixel-aspect-ratio=1/1`;
        break;
  ...
  async _tryPipeline(pStr, timeoutMs, strictLinking = false) {
    ...
    if (needsWait) {
      ...
      while (Date.now() < deadline) {
        ...
        // Sonstige harmlose Warnungen während Preroll ignorieren
        if ((t === 'error' || t === 'warning') &&
            /colorimetry|could.not.get|internal.data.stream|buffer.time|period.time/i.test(txt)) {
          continue;
        }
        ...
        if (t === 'async-done' || t === 'stream-start' || t === 'new-clock' ||
   ...
   * Stops the audio pipeline, rebuilds it with the new preset, seeks to the
   * current video position, and restarts. Causes a brief audio gap (~100ms).
   */
  async reloadAudioPreset(presetId) {
    ...
    // Snapshot current video position for audio re-sync
    let pos = 0;
    try { pos = this.vPipeline.queryPosition() ?? 0; } catch {}
    const itemSomSec = this._item.som ?? 0;
    const absPos = itemSomSec + pos;  // absolute file position

    // Stop old audio bus loop + pipeline (video keeps running).
    // MUSS abgewartet werden, BEVOR die neue Pipeline gebaut wird: beide
    // beanspruchen denselben "interaudiosink channel=<slot>_<groupId>"
    // (gemeinsamer Inter-Pipeline-Ringpuffer, Master liest mit GENAU EINEM
    // interaudiosrc davon). Zwei gleichzeitige Schreiber auf denselben Kanal
    // bringen den Ringpuffer in einen inkonsistenten Zustand → der Master-
    // seitige interaudiosrc blockiert dauerhaft → ALLE Audioausgänge stumm/
    // hängen (genau das gemeldete Symptom "beim Preset-Wechsel stallen alle
    // Audios"). Fire-and-forget (ohne await) ließ exakt dieses Zeitfenster
    // offen, in dem alte und neue interaudiosink-Instanz gleichzeitig liefen.
    const oldAp = this.aPipeline;
    this.aPipeline = null;  // causes _watchAudioBus loop to exit on next iteration
    if (oldAp) await _stopPipeline(oldAp);

    // Update stored item so subsequent ops use the new preset
    this._item = {
      ...this._item,
    ...
    for (const aStr of strategies) {
      ...
      if (ap) {
        if (absPos > 0.01) {
          try { await ap.seek(absPos); } catch(e) { this._log(`Audio seek: ${e.message}`, 'warn'); }
        }
        await ap.play().catch(e => this._log(`Audio play: ${e.message}`, 'warn'));
        this.aPipeline = ap;
        this._watchAudioBus().catch(() => {});
        this._log(`Audio-Preset hot-swap → ${presetId}`);
        return true;
      }
    }
    this._log(`Audio-Preset hot-swap fehlgeschlagen: ${presetId}`, 'warn');
    return false;
  }
    ...
    while (this._busLoop && this.vPipeline) {
      try {
        ...
        if (msg.type === 'element' && msg.srcElementName?.startsWith('level_')) {
          ...
          if (!this._playing) {
            this._log('EOS vor PLAYING — ignoriert', 'debug');
            continue;
          }
          this._log('EOS');
