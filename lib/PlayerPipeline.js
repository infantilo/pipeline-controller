...
class PlayerPipeline extends EventEmitter {
  ...
  async reloadAudioPreset(presetId) {
    ...
    // queryPosition() returns absolute file position (already includes SOM offset)
    let absPos = 0;
    try { absPos = this.vPipeline.queryPosition() ?? 0; } catch {}

    // Keep old pipeline PLAYING while building the new one so the ring buffer
    // never drains. Pausing it immediately would empty the 400 ms PulseAudio
    // buffer before the new pipeline finishes prerolling → underflow.
    ...
    for (const aStr of strategies) {
      ...
      // Preroll on a unique temp channel so the new interaudiosink does NOT
      // join the shared ring buffer while the old pipeline is still writing.
      // Two sinks on the same channel in PAUSED state can corrupt the ring
      // buffer and cause the master-side interaudiosrc to block permanently.
      const chMatch = aStr.match(/interaudiosink channel=(\S+)/);
      const realCh  = chMatch?.[1] ?? null;
      const tempCh  = realCh ? `${realCh}_swap_${Date.now()}` : null;
      const prerollStr = tempCh
        ? aStr.replace(/interaudiosink channel=\S+/, `interaudiosink name=__ia_swap__ channel=${tempCh}`)
        : aStr;

      const ap = await this._tryPipeline(prerollStr, strict ? 4000 : 10000, strict);
      if (ap) {
        // Switch interaudiosink to the real channel while still PAUSED.
        // setElementProperty works on any pipeline state.
        if (tempCh && realCh) {
          try {
            const sinkEl = ap.getElementByName('__ia_swap__');
            if (sinkEl) sinkEl.setElementProperty('channel', realCh);
          } catch(e) { this._log(`channel switch: ${e.message}`, 'warn'); }
        }

        // Get updated position (preroll took time)
        try { absPos = this.vPipeline.queryPosition() ?? absPos; } catch {}

        // Stop old pipeline (fire and forget) — it's no longer needed and
        // must vacate the real channel before new pipeline starts writing.
        if (oldAp) _stopPipeline(oldAp).catch(() => {});

        // Play FIRST — interleave can only forward seek events upstream when
        // the pipeline is in PLAYING state (seeking in PAUSED fails on sink pads).
        ...
        // If state transition is still async, wait until playing() is true
  ...
  async reloadAfd(newAfd) {
    ...
    const vStr = this._buildVideoPipeline(uri, ch, fps);
    const vp   = await this._tryPipeline(vStr, 12000);
    if (!vp) {
