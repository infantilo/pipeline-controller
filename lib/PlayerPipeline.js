...
class PlayerPipeline extends EventEmitter {
  ...
  async load(item) {
    ...
    for (let si = 0; si < aStrategies.length; si++) {
      ...
      if (ap) {
        ...
        aPipeline = ap;
        // gst_element_seek() on a PAUSED mxfdemux+interleave pipeline blocks the
        // main thread indefinitely (pull-mode deadlock). Skip audio seek for these
        // pipelines — same approach as reloadAudioPreset().
        _skipAudioSeek = strict;
        break;
      }
    ...
    if (som > 0.01) {
      try {
        // Audio seek skipped for MXF interleave pipelines — gst_element_seek() on
        // a PAUSED mxfdemux+interleave pipeline blocks the main thread indefinitely.
        await Promise.all([
          vPipeline.seek(som),
          (aPipeline && !_skipAudioSeek) ? aPipeline.seek(som) : Promise.resolve(),
        ]);
        if (_skipAudioSeek) this._log(`Seek → SOM ${toTC(som,fps)} (Audio-Seek übersprungen: MXF-Interleave)`);
        else this._log(`Seek → SOM ${toTC(som,fps)}`);
  ...
  _buildAudioPipeline(uri, ch, fps, audioTracks, audioConfig) {
    ...
    // Kompaktierung: nur die Datei-Kanäle dekodieren/interleaven, die von
    // IRGENDEINEM konfigurierten Preset referenziert werden (nicht nur dem
    // aktiven) — fixiert die Essenzauswahl für die gesamte Clip-Laufzeit, damit
    // jeder spätere On-Air-Preset-Wechsel im zero-gap Matrix-Hotswap-Pfad
    // (reloadAudioPresetMatrix) bleibt statt eines vollen Pipeline-Rebuilds.
    const { tracks, forPreset } = fullTracks
      ? AudioRouter.compactTracksForPresets(fullTracks, Object.values(cfg.presets))
      : { tracks: null, forPreset: (p) => p };
    const routedPreset = forPreset(preset);
  ...
  _buildAudioSource(uri, tracks, caps) {
    ...
    if (isMxf && haveIdx) {
      ...
      tracks.forEach((t, i) => {
        ...
        // subIndices: von compactTracksForPresets gesetzt — welche Sub-Kanäle
  ...
  async _tryPipeline(pStr, timeoutMs, strictLinking = false) {
    ...
    this._log(`PIPELINE-STR: ${pStr}`, 'debug');
  ...
  async reloadAudioPresetMatrix(presetId) {
    ...
    // Same union compaction as _buildAudioPipeline() — guarantees newInCh matches
    // the channel count the running pipeline was already built with, so any switch
    // between configured presets stays in this zero-gap path (no rebuild needed).
    const { tracks: newTracks, forPreset } = fullTracks
      ? AudioRouter.compactTracksForPresets(fullTracks, Object.values(cfg.presets))
      : { tracks: null, forPreset: (p) => p };
    const routedNewPreset = forPreset(newPreset);

    const trackInCh = t => Array.isArray(t.subIndices) ? t.subIndices.length : Math.max(1, t.channels || 1);
    const newInCh = newTracks ? newTracks.reduce((s, t) => s + trackInCh(t), 0) : 2;

    const updates = AudioRouter.computeMatrixUpdates(cfg, routedNewPreset, newInCh, this.slotId);
    if (updates.size === 0) {
  ...
  async reloadAudioPreset(presetId) {
    ...
    let posAtRebuild = null;
    try { posAtRebuild = this.vPipeline.queryPosition(); } catch {}
    this._log(`reloadAudioPreset start: video pos=${posAtRebuild != null ? posAtRebuild.toFixed(3) : '?'}s preset=${presetId}`);
    ...
    for (const aStr of strategies) {
      ...
      // Delay old-pipeline teardown so the new interaudiosink has time to write its
      // first buffers to the gst-inter surface before the old sink's PAUSED→READY
      // transition clears it. Without the delay, the queue drains (~20ms) before
      // the new pipeline produces its first buffer → pulsesink underflow (silence).
      const _oldForStop = oldAp;
      setTimeout(() => { _stopPipeline(_oldForStop).catch(() => {}); }, 300);
  ...
  async reloadAfd(newAfd) {
    ...
    if (absPos > 0.01) {
      try {
        await vp.seek(absPos);
        // Nach einem Flush-Seek muss GStreamer re-prerolling abschließen (async-done)
        // bevor play() aufgerufen wird, sonst startet die Pipeline von Position 0.
        // Entspricht dem Verhalten von load(): dort wird zwischen seek() und go()/play()
        // immer der _watchBus-Loop gestartet der async-done konsumiert.
        const seekDl = Date.now() + 3000;
        while (Date.now() < seekDl) {
          const m = await vp.busPop(200);
          if (!m) break;
          if (m.type === 'async-done') break;
          if (m.type === 'error') { this._log(`Seek-Fehler: ${m.errorMessage || m.message || ''}`, 'warn'); break; }
        }
      } catch(e) { this._log(`Video seek: ${e.message}`, 'warn'); }
    }
    if (!this._playing) {
      // Player wurde während des Rebuilds gestoppt (z.B. Playlist-Advance) — abbrechen
      await _stopPipeline(vp);
      return false;
    }
    ...
    while (this._busLoop && this.vPipeline) {
      try {
        ...
        if (msg.type === 'element' && msg.srcElementName?.startsWith('level_')) {
          ...
          let posAtEos = null;
          try { posAtEos = this.vPipeline.queryPosition(); } catch {}
          this._log(`EOS — video pos=${posAtEos != null ? posAtEos.toFixed(3) : '?'}s item.eom=${this._item?.eom ?? 'null'} playing=${this._playing}`);
    ...
    while (this._busLoop && this.aPipeline) {
      try {
        ...
        if (msg.type === 'element' && msg.srcElementName?.startsWith('level_')) {
          ...
          let aPos = null;
          try { aPos = this.aPipeline?.queryPosition?.(); } catch {}
          this._log(`Audio-EOS (ignoriert) pos=${aPos != null ? aPos.toFixed(3) : '?'}s`, 'debug');
        } else if (msg.type === 'error' || msg.type === 'warning') {
          const txt = msg.errorMessage || msg.warningMessage || msg.message || '';
