...
class ClockStrategy {
  ...
  constructor(clockConfig = {}) {
    ...
    // Analog zu decklinkDevice, aber IP-basiert statt Geräte-Nummer (siehe
    // masterSinkConfig() Fall 'ptp-mtl') — { ip, port, payloadType, channels,
    // sampling, ptime, format }.
    this.st2110Audio         = clockConfig.st2110Audio || null;
    this.st2110AudioExplicit = !!(clockConfig.st2110Audio && clockConfig.st2110Audio.ip);
  ...
  masterSinkConfig(cfg) {
    switch (this.provider) {
      ...
      case 'ptp-mtl':
        // Clock-Domäne = Intel MTL-Kontext (E810, PTP-synced über die NIC-Hardware-
        // Clock) via Sink-Clock-Auto-Selection — exakt wie ptp-decklink oben: der
        // mtl_st20p_tx/mtl_st30p_tx-Sink in der Master-Pipeline ist der einzige
        // Clock-Provider, kein setClock nötig (gst-kit hat ohnehin keine Clock-API).
        if (this.st2110AudioExplicit) {
          // Primäre Gruppe → mtl_st30p_tx (liefert die MTL-Clock, auch wenn der
          // Video-Sink kein MTL ist). Siehe AudioRouter._buildSink type 'mtl'.
          return { type: 'mtl', st2110: this.st2110Audio, bufferTime: this.bufferTime };
        }
        console.warn('[ClockStrategy] ptp-mtl: kein st2110Audio konfiguriert — MTL-Video-Sink liefert die Pipeline-Clock, Audio bleibt auf Pulse');
        return { type: 'pulse', bufferTime: this.bufferTime, noProvideClock: true };
