'use strict';
/**
 * PluginHost — lädt und verwaltet Plugins in isolierten Child-Prozessen.
 * Verwendet child_process.fork() statt Worker-Threads, da gst-kit's
 * Native-Addon (gst_kit.node) den worker_thread-Bootstrap in Node.js 25
 * durch Memory-Corruption verhindert (ERR_STRING_TOO_LONG).
...
class PluginHost extends EventEmitter {
  ...
  // ── Worker-Lifecycle (Child-Process) ─────────────────────────────────────────
  async _startWorker(entry) {
    ...
    // Initialisierungsdaten als erste Nachricht senden
    child.send({
      type:       'init',
    ...
    child.on('error', err => {
      this._log(`[${entry.id}] Prozess-Fehler: ${err.message}`, 'warn');
      entry.worker = null;
    ...
    child.on('exit', (code, signal) => {
      if (entry.worker !== child) return;
      entry.worker = null;
      const exitDesc = signal ? `Signal ${signal}` : `Exit-Code ${code}`;
      if ((code !== 0 || signal) && entry.enabled) {
        entry.status = { state: 'error', error: exitDesc, data: entry.status.data };
        this._broadcastStatus(entry);
        this._scheduleRestart(entry);
  ...
  async _stopWorker(entry, markDisabled = true) {
    ...
    try {
      ...
      w.kill('SIGTERM');
      await new Promise(r => { w.once('exit', r); setTimeout(r, 2000); });
    } catch {}
    if (markDisabled) {
  ...
  _handleApiCall(entry, msg) {
    ...
    try { entry.worker?.send({ type: 'api-result', id: msg.id, value }); } catch {}
  }
  ...
  dispatch(eventName, data) {
    ...
    if (payload === null && data !== null && data !== undefined) return;
    ...
    for (const [, entry] of this._plugins) {
      ...
      try {
        entry.worker.send({ type: 'event', name: eventName, data: payload });
      } catch (e) {
  ...
  /** Konfiguration eines Plugins setzen und Prozess neu starten */
