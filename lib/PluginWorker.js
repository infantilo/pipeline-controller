...
process.once('message', async initMsg => {
  ...
  const api = {
    ...
    // sync (optional): { targetEventId, schedStartMs, toleranceSec } — für framegenauen
    // On-Air-Abgleich, wenn die Playlist bereits läuft (siehe marina-sync Plugin).
    setPlaylist(events, startIndex = null, somOffset = null, sync = null) {
      _send({
        type:       'action',
        action:     'playlist:set',
        ...
        sync:       (sync && typeof sync === 'object') ? _safeClone(sync) : null,
      });
    },
