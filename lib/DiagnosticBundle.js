...
// ANSI-Farbcodes können GStreamer-seitig um JEDES Feld einzeln liegen (Zeit,
// PID, Level, Kategorie …), nicht nur am Zeilenanfang — passiert wenn GST_DEBUG
// aus der geerbten Shell-Umgebung kommt statt aus settings.json und dadurch
// OHNE GST_DEBUG_NO_COLOR=1 lief. Statt die Regex an alle möglichen Positionen
// anzupassen: Escapes vor dem Test einfach vollständig entfernen.
...
function decklinkLogTrace(workDir) {
  try {
    ...
    fs.closeSync(fd);
    const allText = _stripAnsi(buf.toString('utf8'));
...
function collect({ settings = {}, masterOpts = {}, master = null, debugger_ = null, logs = [], workDir }) {
  ...
  // gstDebugFilter (gespeichert) vs. gstDebugActive (process.env.GST_DEBUG in
  // DIESEM Prozess) bewusst getrennt ausgewiesen: applyGstDebugEarly() in
  // server.js überspringt das Laden aus settings.json komplett, wenn GST_DEBUG
  // schon aus der Umgebung gesetzt ist (z.B. Rest aus der Start-Shell/einem
  // alten .bashrc-Export) — dann weicht "aktiv" vom gespeicherten Server-Tab-
  // Wert ab, egal wie oft dieser neu gespeichert wird. Ohne diese Zeile ist das
  // aus dem Bundle nicht erkennbar.
  parts.push(section('Master-Konfiguration', JSON.stringify({
    ...
    gstDebugActive: process.env.GST_DEBUG || '(nicht gesetzt — GST_DEBUG war beim Prozessstart leer)',
  }, null, 2)));
