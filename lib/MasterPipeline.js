...
const { isInterlacedMode, interlaceChainStr } = require('./InterlaceChain');

const LABELS  = { 0: 'PLAYER1', 1: 'PLAYER2', 2: 'SMPTE', 3: 'BLACK', 4: 'IMAGE', 5: 'IMAGE_PL', 6: 'PLAYER_IDLE' };

// "mxl://<domain>?video=<flow-uuid>&audio=<flow-uuid>" — eigene Konvention dieses
// Projekts (NICHT die offizielle MXL-URI-Spec), siehe lib/MxlSource.js. Liest aus
// einer lokalen MXL-Domain (Shared-Memory-Verzeichnis) per gst-mxl-rs "mxlsrc".
...
class MasterPipeline extends EventEmitter {
  constructor(opts = {}) {
    ...
    // live-source id → {message, ts}. Absichtlich NICHT in stop() zurückgesetzt — überlebt
    // Pipeline-Rebuilds (z.B. beim Speichern der Live-Quellen), damit ein einmal als defekt
    // erkannter DeckLink-Kanal (z.B. E_ACCESSDENIED/"in use already" — siehe Bug: ein Kanal,
    // der nicht sauber stoppt, kann den Freigabe-Handshake für ALLE anderen DeckLink-Kanäle
    // auf derselben Karte blockieren) nie wieder eingebettet wird und so nie wieder die
    // ganze Pipeline mit sich reißen kann. Nur clearLiveSourceBroken() (manueller Retry aus
    // der UI) nimmt den Ausschluss zurück.
    this._liveSourceBroken    = {};
  }

  isLiveSourceBroken(id)  { return !!this._liveSourceBroken[id]; }
  clearLiveSourceBroken(id) { delete this._liveSourceBroken[id]; }

  markLiveSourceBroken(id, message) {
    const wasBroken = !!this._liveSourceBroken[id];
    this._liveSourceBroken[id] = { message, ts: Date.now() };
    // Nur beim ERSTEN Erkennen loggen/emittieren — dieselbe kaputte Hardware-Quelle
    // kann pro Sekunde mehrfach denselben Bus-Error posten, bevor der nächste
    // Pipeline-Aufbau sie durch den Platzhalter ersetzt.
    if (!wasBroken) {
      this._log(`Live-Quelle "${id}": als defekt markiert (${message}) — wird ab dem nächsten Pipeline-Aufbau durch Platzhalter ersetzt, bis manuell zurückgesetzt`, 'warn');
      this.emit('decklink-signal', { id, ok: false, error: message, ts: Date.now() });
      this.emit('live-source-broken', { id, message });
    }
  }
  ...
  _detectVideoSink() {
    if (this.opts.videoSink) {
      const sinkName = this.opts.videoSink.split(/\s/)[0];
      const needsDisplay = ['ximagesink','xvimagesink','glimagesink','waylandsink'].includes(sinkName);
      if (needsDisplay && !process.env.DISPLAY && !process.env.WAYLAND_DISPLAY) {
        this._log(`videoSink="${sinkName}" benötigt DISPLAY — kein X11/Wayland verfügbar, falle auf fakesink zurück`, 'warn');
        return 'fakesink sync=false';
      }
      return this.opts.videoSink;
    }
  ...
  // Rate-Stufe vor dem PGM-Video-Sink: Standard = videorate (Passthrough/Anpassung).
  // Ist der Sink ein decklinkvideosink mit interlaced Mode (1080i50/pal/…), wird die
  // gemeinsame interlace-Kette eingefügt (lib/InterlaceChain.js — gleiche Kette wie
  // in OutputEngine): videorate auf Feldrate + interlace 1:1 → interleaved Frames.
  _vsinkRateStage(videoSink) {
    const m = /decklinkvideosink\b[^!]*\bmode=([\w-]+)/.exec(videoSink || '');
    if (m && isInterlacedMode(m[1])) {
      this._log(`PGM-Sink interlaced Mode "${m[1]}" → interlace-Kette aktiv`);
      return interlaceChainStr(m[1]);
    }
    return 'videorate';
  }
  ...
  build() {
    ...
    // Colorimetry: HD (>=720 Zeilen) = bt709, SD = bt601 — bt601 auf HD-Auflösung ist
    // farblich falsch (relevant für DeckLink/2110-Ausgabe, sichtbarer Farbstich).
    const colorimetry = height >= 720 ? 'bt709' : 'bt601';
    const vcaps      = `video/x-raw,width=${width},height=${height},framerate=${fps}/1,colorimetry=${colorimetry}`;
    ...
    const pipe = [
      ...
      `! tee name=vtee`,
      `vtee. ! queue leaky=downstream max-size-buffers=8 ! videoconvert ! videoscale ! ${this._vsinkRateStage(videoSink)} ! ${videoSink} name=vsink`,
      ...
      // DVE/Squeeze: verkleinert+positioniert das Programmbild (Clip/Live, was auch
      // immer gerade auf isel aktiv ist) innerhalb des vollen Frames.
      // gst-kit kann GstCompositorPad-Properties (sink_N::xpos/width/...) NICHT zur
      // Laufzeit setzen (nur zur Parse-Zeit) — daher hier stattdessen zwei reine
      // Element-Properties: capsfilter.caps (Zielgröße) + videobox (Padding/Position).
      // Default = Passthrough (volle Auflösung, kein Crop/Border) → keine Zusatzkosten
      // wenn kein Squeeze aktiv ist (BaseTransform passthrough-Fastpath).
      `! videoscale name=dveScale`,
      `! capsfilter name=dveCaps caps="${rgba}"`,
      `! videobox name=dveBox left=0 right=0 top=0 bottom=0 border-alpha=0.0 alpha=1.0`,
      ...
      ...this._liveSources.flatMap((ls, i) => {
        ...
        // Als defekt markierte Quelle (siehe markLiveSourceBroken() / _watchBus()) NIE mehr
        // mit ihrem echten gstSrc einbetten — ein Kanal, der beim Start/Stop scheitert, kann
        // sonst die ganze Karte (und damit alle anderen DeckLink-Rollen im PGM-Pfad) blockieren.
        // Reiner videotestsrc-Platzhalter statt dessen: keine Hardware, kann nicht mehr scheitern.
        if (this.isLiveSourceBroken(ls.id)) {
          delete this._dlSigNames[ls.id];
          return [
            `videotestsrc is-live=true pattern=black ! ${vcaps} ! queue leaky=downstream max-size-buffers=3 ! isel.sink_${padIdx}`,
  ...
  _liveSourceAudioPads(padIdx, ls, acaps) {
    if (this.isLiveSourceBroken(ls.id)) {
      // Wie eine Quelle ohne Audio: im AudioGroupConfig-Modus reicht [] — AudioRouters
      // interaudiosrc-Reader liefert automatisch Stille, wenn kein interaudiosink-Writer
      // für diesen Kanal existiert (siehe Kommentar unten). Im Legacy-Modus müssen die
      // aisel_pgm/aisel_sec-Pads trotzdem existieren (fixe Pad-Indizierung), daher explizite
      // Stille statt der echten (defekten) Hardware-Quelle.
      if (this.audioGroupConfig) return [];
      const q = `queue max-size-buffers=0 max-size-time=500000000 max-size-bytes=0`;
      return [
        `audiotestsrc wave=silence is-live=true ! ${acaps} ! ${q} ! aisel_pgm.sink_${padIdx}`,
        `audiotestsrc wave=silence is-live=true ! ${acaps} ! ${q} ! aisel_sec.sink_${padIdx}`,
      ];
    }
    if (this.audioGroupConfig) {
  ...
  async start() {
    ...
    if ((r.result === 'failure' && (r.finalState == null || r.finalState < 3)) ||
      ...
      // Fehlgeschlagene Pipeline explizit stoppen statt dem JS-GC zu überlassen —
      // bei DeckLink-Sinks kann ein noch offenes/nicht freigegebenes Handle einen
      // unmittelbar folgenden Neuversuch (Caller-Retry oder erneuter start()-Aufruf)
      // erst recht scheitern lassen ("Karte bereits von altem Handle belegt").
    ...
    if (r.result === 'async' || r.result === 'failure' || r.finalState === 3) {
      ...
      while (Date.now() < deadline) {
        ...
        if (m.type === 'error' || m.errorMessage) {
          // Fehler von einer eingebetteten DeckLink-Live-Quelle (dlsig_*) darf den ASYNC-Wait
          // NICHT abbrechen — das ist nur diese eine Quelle, nicht die Gesamt-Pipeline. Als
          // defekt markieren (→ Platzhalter ab dem nächsten Aufbau) und normal weiterwarten.
          const brokenId = Object.entries(this._dlSigNames).find(([, elName]) => elName === m.srcElementName)?.[0];
          if (brokenId) { this.markLiveSourceBroken(brokenId, m.errorMessage || m.type || 'Bus-Error ohne Text'); continue; }
          this._log(`Async-Fehler: ${m.errorMessage || m.type}`, 'warn'); break;
        }
    ...
    this._dveScale   = this.pipeline.getElementByName('dveScale');
    this._dveCaps    = this.pipeline.getElementByName('dveCaps');
    this._dveBox     = this.pipeline.getElementByName('dveBox');
  ...
  async startLiveFeeder(ls) {
    ...
    // Colorimetry muss zur Master-vcaps in build() passen (HD=bt709, SD=bt601),
    // sonst Fehlinterpretation der Farbmatrix an der intervideosink/src-Grenze.
    const vcaps = `video/x-raw,width=${width},height=${height},framerate=${fps}/1,colorimetry=${height >= 720 ? 'bt709' : 'bt601'}`;
  ...
  async stop() {
    ...
    this._dveScale = this._dveCaps = this._dveBox = null;
  ...
  // ── DVE/Squeeze ───────────────────────────────────────────────────────────
  // Verkleinert + positioniert das komplette Programmbild (was auch immer gerade
  // auf isel aktiv ist — Player-Clip, Live-Quelle, SMPTE, Bild …) innerhalb des
  // vollen Frames. Wirkt NUR auf den fg-Pfad (comp.sink_0) — Grafik-Overlay
  // (comp.sink_2) und Audio bleiben unberührt, A/V-Sync somit unverändert.
  //
  // Funktioniert über zwei reine Element-Properties (videoscale-Zielgröße via
  // capsfilter.caps + videobox-Padding) statt GstCompositorPad-Properties
  // (sink_0::xpos etc.) — letztere kann gst-kit zur Laufzeit NICHT setzen
  // (nur gst_parse_launch zur Parse-Zeit). border-alpha=0 macht den Rand um
  // das verkleinerte Bild transparent, sodass dahinter z.B. ein Grafik-Rahmen
  // (Lower-Third-Child mit eigenem dve-Feld) sichtbar bleibt.
  setDveSqueeze(box) {
    if (!this._dveCaps || !this._dveBox) return;
    const W = this.opts.width || 1920, H = this.opts.height || 1080, fps = this.opts.fps || 25;
    const w = Math.max(2, Math.min(W, Math.round((box?.w || W) / 2) * 2));
    const h = Math.max(2, Math.min(H, Math.round((box?.h || H) / 2) * 2));
    const x = Math.max(0, Math.min(W - w, Math.round(box?.x || 0)));
    const y = Math.max(0, Math.min(H - h, Math.round(box?.y || 0)));
    try {
      this._dveCaps.setElementProperty('caps', `video/x-raw,format=RGBA,width=${w},height=${h},framerate=${fps}/1`);
      this._dveBox.setElementProperty('left',   -x);
      this._dveBox.setElementProperty('top',    -y);
      this._dveBox.setElementProperty('right',  -(W - x - w));
      this._dveBox.setElementProperty('bottom', -(H - y - h));
      this._log(`DVE-Squeeze aktiv: ${w}×${h} @ (${x},${y})`, 'debug');
    } catch(e) { this._log(`setDveSqueeze: ${e.message}`, 'warn'); }
  }
  ...
  clearDveSqueeze() {
    ...
    try {
      this._dveCaps.setElementProperty('caps', `video/x-raw,format=RGBA,width=${W},height=${H},framerate=${fps}/1`);
      this._dveBox.setElementProperty('left', 0);
      this._dveBox.setElementProperty('top', 0);
      this._dveBox.setElementProperty('right', 0);
      this._dveBox.setElementProperty('bottom', 0);
      this._log('DVE-Squeeze zurückgesetzt (Vollbild)', 'debug');
    } catch(e) { this._log(`clearDveSqueeze: ${e.message}`, 'warn'); }
  }
    ...
    while (this._busLoop && this.pipeline) {
      try {
        ...
        if (msg.type === 'error') {
          ...
          // Bus-Error von einer eingebetteten DeckLink-Live-Quelle (dlsig_*-Element,
          // siehe build()): egal welcher Fehlertext — dieser Kanal hat gerade bewiesen,
          // dass er nicht sauber läuft. Da ein solcher Kanal im Beobachtungsfall auch
          // beim Stoppen hängen bleibt und dabei die Freigabe ANDERER DeckLink-Rollen
          // (z.B. den PGM-Ausgang auf Karte 0) blockieren kann, wird er sofort als
          // defekt markiert statt nur geloggt — build() ersetzt ihn beim nächsten
          // Pipeline-Aufbau durch einen harmlosen Platzhalter (siehe markLiveSourceBroken()).
          const brokenId = Object.entries(this._dlSigNames).find(([, elName]) => elName === msg.srcElementName)?.[0];
          if (brokenId) { this.markLiveSourceBroken(brokenId, txt || 'Bus-Error ohne Text'); continue; }
