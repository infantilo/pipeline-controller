...
class MasterPipeline extends EventEmitter {
  ...
  _liveSourceAudioPads(padIdx, ls, acaps) {
    if (this.audioGroupConfig) {
      ...
      if (!(ls.hasAudio && ls.gstAudioSrc)) return [];

      const cfg      = this.audioGroupConfig;
      // ls.audioConfig.preset: gleicher Preset-Mechanismus wie bei Playern
      // (audio_config.json → routes mit "mxf_chN"). Bei Live-Quellen
      // adressiert "mxf_chN" den N-ten Audio-Kanal der Live-Quelle (z.B.
      // SDI-Embedded-Audio-Gruppen/-Paare über decklinkaudiosrc, oder die
      // Kanäle eines Stream-URI). ls.audioChannels gibt deren Gesamtzahl an
      // (Default 2 = Stereo, z.B. bei einfachen URI-Quellen).
      const presetId = ls.audioConfig?.preset || null;
      const preset   = presetId ? cfg.getPreset(presetId) : null;
      const inCh     = Math.max(1, ls.audioChannels || 2);

      // Kein Preset gewählt → Status quo: Direkt-Downmix auf die PGM-Gruppe
      // (audioconvert übernimmt Up-/Downmix). Andere Gruppen bleiben leer
      // (Silence-Seeder hält den Channel aktiv) — analog zu "Kein Mapping"
      // bei Playern.
      if (!preset) {
        const pgmGroup = cfg.groups[0];
        if (!pgmGroup) return [];
        const ch    = `${ls.id}_${pgmGroup.id}`;
        const gCaps = `audio/x-raw,format=F32LE,rate=48000,channels=${pgmGroup.channels},layout=interleaved`;
        return [
      ...
      // Preset gewählt → exakt dieselbe Routing-Logik wie auf der Player-Seite:
      // EINE Dekodierung der Live-Quelle als inCh-kanaliger interleaved Strom,
      // tee → je Branch pro gemappter Gruppe mit eigener audiomixmatrix.
      ...
      return [
        `${ls.gstAudioSrc} ! audioconvert ! audioresample ! ${liveCaps} ! tee name=${teeName}`,
