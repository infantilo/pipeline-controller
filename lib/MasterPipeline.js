...
function _buildUriSrc(uri) {
  if (!uri) return null;
  if (/^rtsp:\/\//i.test(uri))  return `rtspsrc location="${uri}" latency=200 ! decodebin ! videoconvert`;
  if (/^rtsps:\/\//i.test(uri)) return `rtspsrc location="${uri}" latency=200 tls-validation-flags=0 ! decodebin ! videoconvert`;
...
class MasterPipeline extends EventEmitter {
    ...
    const pipe = [
      ...
      ...this._liveSources.flatMap((ls, i) => {
        ...
        const dlFmt  = (_isEmbedded(ls) && /decklink/i.test(ls.gstSrc || '') && ls.videoFormat)
          ? ` video-format=${ls.videoFormat}` : '';
        const vSrc   = _isEmbedded(ls)
          ? `${ls.gstSrc}${dlFmt} ! videoconvert`
  ...
  _liveSourceAudioPads(padIdx, ls, acaps) {
    if (this.audioGroupConfig) {
      ...
      if (!preset) {
        ...
        return [
          `${gstAudioSrc} ! audioconvert ! audioresample ! ${gCaps}` +
  ...
  async startLiveFeeder(ls) {
    ...
    await this.stopLiveFeeder(ls.id);
    const { width = 1920, height = 1080, fps = 25 } = this.opts;
    const vcaps = `video/x-raw,width=${width},height=${height},framerate=${fps}/1,colorimetry=bt601`;
    const vCh   = `${ls.id}_live`;
    const videoSrc = ls.gstSrc || _buildUriSrc(ls.uri);
    ...
    const audioSrc = ls.audioSources?.[0]?.gstSrc || ls.gstAudioSrc || null;
    ...
    while (this._busLoop && this.pipeline) {
      try {
        ...
        if (msg.type === 'element') {
          ...
          // DeckLink signal state changes from embedded live sources
          if (/decklink/i.test(msg.srcElementName || '')) {
            const structure = msg.structure || msg.structureName || '';
            const ok = !/no.signal|signal.lost|no-signal/i.test(structure);
            // Extract device number from element name suffix, e.g. "decklinkvideosrc0" → 0
            const slotMatch = (msg.srcElementName || '').match(/(\d+)$/);
            const slot = slotMatch ? parseInt(slotMatch[1]) : 0;
            this.emit('decklink-signal', { slot, ok, structure });
          }
          continue;
        }
        if (msg.type === 'error') {
