...
function classifyAudio(tracks, audioConfig = {}) {
  ...
  return tracks.map((t, i) => {
    const ch  = t.channels || 1;
    return {
      ...t,
      role:    merged[i] ?? seq[i] ?? fallback,
      chLabel: t.chLabel || (ch === 1 ? '1ch' : ch === 2 ? '2ch' : `${ch}ch`),
    };
  });
}
...
function parseFfprobeResult(data) {
  ...
  for (const s of (data.streams || [])) {
    if (s.codec_type === 'video' && !result.video) {
      ...
      // MXF / MOV track descriptions live in handler_name or title tag
      const trackTitle = s.tags?.title || s.tags?.handler_name || null;
      result.audio.push({
        trackIndex:  audioIdx++,
        streamIndex: s.index,   // absolute ffprobe stream index — needed to address db.src_<N> pads
        channels:   ch,
        rate:       parseInt(s.sample_rate) || 48000,
        codec:      s.codec_name || null,
        bitrate:    s.bit_rate ? parseInt(s.bit_rate) : null,
        chLabel:    ch === 1 ? '1ch' : ch === 2 ? '2ch' : `${ch}ch`,
