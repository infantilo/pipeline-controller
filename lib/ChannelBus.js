...
class ChannelBus extends EventEmitter {
  ...
  _handleLine(sock, meta, line) {
    ...
    if (msg.t === 'hello') {
      // Schutz gegen Eigen-Loop: eine (typischerweise versehentliche) Verbindung
      // zu sich selbst (eigener Host:Port in `peers`) würde sonst echte Trigger
      // an sich selbst zurückspiegeln. Verbindung sofort verwerfen.
      if (msg.id === this.id) {
        this._log(`ChannelBus: Selbst-Verbindung erkannt (id "${msg.id}") — wird verworfen`, 'warn');
        this._sockets.delete(sock);
        sock.destroy();
        return;
      }
